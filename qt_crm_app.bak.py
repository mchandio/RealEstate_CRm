...
from crm_core.constants import EXPENSE_CATEGORIES, normalize_availability_status, normalize_contact_role
...
try:
    ...
    from PySide6.QtCore import QDate, QEvent, QMarginsF, QPointF, QRectF, Qt, QTimer, Signal
...
GLOBAL_SEARCH_SOURCES = [
    ...
    ("Client", "clients"),
    ("Brocker Contact", "broker_contacts"),
    ("Property", "properties"),
    ("Employee", "employees"),
...
class DataTablePage(QWidget):
    ...
    def clear_filters(self) -> None:
        widgets = (self.keyword_input, self.sort_combo, self.direction_combo, self.start_date, self.end_date)
...
def phase1_section_specs(main: "ModernCRMWindow") -> dict[str, PhaseOneSectionSpec]:
    ...
    common_req_tail = [
        ...
        FieldSpec("Floor *", "floor", "combo", options=floors, required=True),
    ...
    def _read_import_file(self, path: str) -> list[dict]:
        ...
        for source in source_rows:
            ...
            for field in self.spec.fields:
                mapped.setdefault(field.key, "")
                if field.numeric:
                    mapped[field.key] = safe_float(mapped[field.key])
...
class PhaseOneDesk(QWidget):
    ...
    def _match_score(self, req: dict, row: dict, target: PhaseOneSectionSpec) -> tuple[float, list[str]]:
        ...
        if normalize_text(req.get("floor")) and normalize_text(req.get("floor")) == normalize_text(row.get("floor")):
            score += 10
...
class AttendancePage(QWidget):
    ...
    def refresh(self) -> None:
        ...
        marked = self.main.services.fetch_all(
            f"""SELECT a.id, e.id AS employee_id, e.full_name, a.date, a.status, a.notes
        ...
        if marked or start != end:
            ...
            self.rows = [
                {"id": None, "employee_id": row["id"], "full_name": row["full_name"], "date": start, "status": "Not Marked", "notes": ""}
...
class ModernCRMWindow(QMainWindow):
    ...
    def _build_menu(self) -> None:
        ...
        add_record_action(records_new_menu, "Brocker Contact", "broker_contacts", None)
    ...
    def _build_pages(self) -> None:
        ...
        if record_pages:
            ...
            if "broker_contacts" in record_pages:
                ...
                self._add_page("broker_contacts", "Brocker Contact List", DataTablePage(self, self.specs["broker_contacts"]))
        if has_permission(self.role, "financial") or has_permission(self.role, "financial_view"):
    ...
    def _dashboard_summary_data(self) -> dict[str, Any]:
        rent_requirements = self._dashboard_count("rent_requirements")
        rent_available = self._dashboard_count("rent_availability")
        rented_done = self._dashboard_count("rented_properties")
        sale_requirements = self._dashboard_count("sale_requirements")
        sale_available = self._dashboard_count("sale_availability")
        sold_done = self._dashboard_count("sold_properties")
        clients = self._dashboard_count("clients")
        pending = self._dashboard_pending_approvals()
        total_deals = rent_requirements + rent_available + sale_requirements + sale_available + rented_done + sold_done
        conversion = round((self._dashboard_closed_count() / total_deals) * 100) if total_deals else 0
        approval_clearance = max(10, min(90, 100 - min(pending, 90)))
        return {
            "kpis": [
                ("Rent Requirements", rent_requirements, "blue"),
                ("Rent Availability", rent_available, "cyan"),
                ("Sale Requirements", sale_requirements, "silver"),
                ("Sale Availability", sale_available, "green"),
                ("Rented Done", rented_done, "royal"),
                ("Sold Done", sold_done, "sky"),
                ("Properties", self._dashboard_count("properties"), "royal"),
                ("Clients", clients, "sky"),
                ("Employee", self._dashboard_count("employees"), "slate"),
            ],
            "pending": pending,
            "locations": self._dashboard_location_buckets(),
            "segments": self._dashboard_client_segments(clients),
            "clients": clients,
            "roadmap": [
                {"period": "30 Days", "response_time": 42, "approvals_cleared": max(20, approval_clearance - 20), "conversion": max(12, conversion)},
                {"period": "90 Days", "response_time": 70, "approvals_cleared": max(50, approval_clearance), "conversion": max(30, conversion + 12)},
                {"period": "180 Days", "response_time": 66, "approvals_cleared": max(75, min(95, approval_clearance + 18)), "conversion": max(45, conversion + 25)},
            ],
            "operations": [
                ("First Response", "< 15 Min", "#0e82b1"),
                ("Pending Approvals", f"{pending} Needs Review", "#ff9818"),
                ("Conversion Rate", f"{conversion}%", "#43a047"),
            ],
        }
...
def deal_common_fields(
    ...
    *,
    ...
    return [
        ...
        FieldSpec("Floor", "floor", "combo_other", options=floors),
        FieldSpec("Location *", "location", "autocomplete", options=areas, required=True),
        FieldSpec("Facilities", "facilities", "facilities", options=facilities),
...
def broker_contact_spec() -> TableSpec:
    ...
    insert = ["name", "contact", "area", "remarks", "created_at"]
    update = ["name", "contact", "area", "remarks"]
    return TableSpec(
        "Brocker Contact List",
        "broker_contacts",
        cols,
        fields,
        insert,
