...
from crm_core import AI_LIBS_AVAILABLE, APP_ROOT, OUTPUT_DIR, IntelligenceService
DB_PATH = APP_ROOT / "real_estate_crm_exp.db"
os.environ.setdefault("CRM_DB_PATH", str(DB_PATH))
...
try:
    ...
    from PySide6.QtCore import QDate, QEvent, QMarginsF, QRectF, QSizeF, Qt, QTimer, Signal
    from PySide6.QtGui import QAction, QColor, QFont, QIcon, QPageLayout, QPageSize, QPainter, QPixmap, QAbstractTextDocumentLayout, QTextDocument
    from PySide6.QtPrintSupport import QPrintDialog, QPrintPreviewDialog, QPrintPreviewWidget, QPrinter
...
REPORT_PREVIEW_FONT_SIZE = 11
REPORT_PRINT_FONT_SIZE = 9
...
def configure_lan_database_environment() -> None:
    """Keep the LAN browser server on the same SQLite file as this desktop app."""
    os.environ["CRM_DB_PATH"] = str(DB_PATH)
    os.environ.setdefault("DATABASE_URL", f"sqlite:///{DB_PATH.as_posix()}")


def report_preview_font() -> QFont:
    return QFont("Consolas", REPORT_PREVIEW_FONT_SIZE)
...
def _first_report_value(row: dict, *keys: str) -> Any:
    for key in keys:
...
def _clip_report_text(value: Any, max_chars: int = 90) -> str:
    text = str(value or "").strip()
...
def _report_text_meta(text: str, label: str) -> str:
...
def _summary_value_for_print(key: str, value: Any) -> str:
    if isinstance(value, dict):
        return ""
    if any(token in key for token in ("budget", "rent", "demand", "income", "expense", "profit", "amount")):
        return money(value)
...
def _summary_label_for_print(key: str) -> str:
    labels = {
        "requirements": "Requirements",
        "available_properties": "Availability",
        "total_requirement_budget": "Total Budget",
        "average_requirement_budget": "Avg Budget",
        "total_monthly_rent": "Total Rent",
        "average_monthly_rent": "Avg Rent",
        "total_owner_demand": "Total Demand",
        "average_owner_demand": "Avg Demand",
    }
    return labels.get(key, key.replace("_", " ").title())


def _summary_cards_html(result: ReportResult) -> str:
    summary = result.summary or {}
    cards: list[tuple[str, str]] = []
    if "rent" in summary or "sale" in summary:
        rent_summary = summary.get("rent") or {}
        sale_summary = summary.get("sale") or {}
        for label, data in (("Rent", rent_summary), ("Sale", sale_summary)):
            if not isinstance(data, dict):
                continue
            cards.extend(
                [
                    (f"{label} Requirements", _summary_value_for_print("requirements", data.get("requirements", 0))),
                    (f"{label} Availability", _summary_value_for_print("available_properties", data.get("available_properties", 0))),
                    (f"{label} Total Budget", _summary_value_for_print("total_requirement_budget", data.get("total_requirement_budget", 0))),
                ]
            )
            amount_key = "total_monthly_rent" if label == "Rent" else "total_owner_demand"
            amount_label = "Total Rent" if label == "Rent" else "Total Demand"
            cards.append((f"{label} {amount_label}", _summary_value_for_print(amount_key, data.get(amount_key, 0))))
    else:
        preferred = [
            "requirements",
            ...
            "total_owner_demand",
            "average_owner_demand",
        ]
        for key in preferred:
            if key in summary:
                cards.append((_summary_label_for_print(key), _summary_value_for_print(key, summary[key])))
        for key, value in summary.items():
            if key not in preferred and not isinstance(value, dict):
                cards.append((_summary_label_for_print(key), _summary_value_for_print(key, value)))
    ...
    return "<table class=\"summary\" cellspacing=\"0\" cellpadding=\"0\">" + "".join(rows) + "</table>"


def _amount_for_report_row(row: dict) -> str:
    for key, label in (("budget", "Budget"), ("monthly_rent", "Rent"), ("demand", "Demand"), ("amount", "Amount")):
        if key in row and row.get(key) not in (None, ""):
            return f"{label}: {money(row.get(key))}"
    return "-"


def _property_for_report_row(row: dict) -> str:
    return str(_first_report_value(row, "property_requires", "property_availability", "property_type") or "-")


def _detail_for_report_row(row: dict) -> str:
    details = []
    size = _first_report_value(row, "size", "measurement")
    floor = _first_report_value(row, "floor", "floor_no")
    if size:
...
def _report_rows_html(result: ReportResult) -> str:
    if not result.rows:
        return ""

    grouped: dict[str, list[dict]] = {}
    for row in result.rows:
        section = str(row.get("section") or "Records").strip() or "Records"
        grouped.setdefault(section, []).append(row)

    preferred_order = ["Rent Requirement", "Rent Availability", "Sale Requirement", "Sale Availability"]
    ordered_sections = [section for section in preferred_order if section in grouped]
    ordered_sections.extend(section for section in grouped if section not in ordered_sections)

    widths = ["5%", "9%", "15%", "11%", "16%", "10%", "13%", "11%", "10%"]
    headers = ["Sr", "Date", "Name", "Contact", "Property", "Size/Floor", "Location", "Amount", "Stage"]
    parts: list[str] = []
    for section in ordered_sections:
        rows = grouped[section]
        parts.append(f"<div class=\"section-title\">{_html_text(section)} <span>{len(rows):,} records</span></div>")
        parts.append("<table class=\"records\" cellspacing=\"0\" cellpadding=\"0\" width=\"100%\">")
        parts.append(
            "<tr>"
            + "".join(f"<th width=\"{width}\">{_html_text(label)}</th>" for width, label in zip(widths, headers))
            + "</tr>"
        )
        for index, row in enumerate(rows):
            row_class = " class=\"alt\"" if index % 2 else ""
            name = _first_report_value(row, "client_name", "owner_name", "full_name", "name", "title")
            stage = _first_report_value(row, "workflow_stage", "status", "approval_status") or "Lead"
            cells = [
                f"#{_html_text(row.get('id') or '-')}",
                _html_text(format_date_display(row.get("date"))),
                _html_text(_clip_report_text(name, 32) or "-"),
                _html_text(_clip_report_text(_first_report_value(row, "contact", "phone", "contact_phone"), 20) or "-"),
                _html_text(_clip_report_text(_property_for_report_row(row), 34)),
                _html_text(_clip_report_text(_detail_for_report_row(row), 24)),
                _html_text(_clip_report_text(row.get("location"), 32) or "-"),
                _html_text(_amount_for_report_row(row)),
                _html_text(_clip_report_text(stage, 22)),
            ]
            parts.append("<tr" + row_class + ">" + "".join(f"<td>{cell}</td>" for cell in cells) + "</tr>")
            remarks = _clip_report_text(_first_report_value(row, "remarks", "description", "notes"), 150)
            if remarks:
                parts.append(
                    f"<tr{row_class}><td></td><td colspan=\"8\" class=\"remarks\">Remarks: {_html_text(remarks)}</td></tr>"
                )
        parts.append("</table>")
    return "".join(parts)


def _plain_report_html(text: str) -> str:
    lines = []
    for line in (text or "").splitlines():
        clean = line.strip()
        if clean and set(clean) <= {"=", "-"}:
            continue
        lines.append(line)
    clean_text = "\n".join(lines).strip()
    return f"<pre class=\"plain-report\">{html.escape(clean_text)}</pre>"


def _report_print_html(result: ReportResult, font_size_pt: int) -> str:
    css_size = f"{font_size_pt}pt"
    small_size = f"{max(font_size_pt - 1, 7)}pt"
    summary_html = _summary_cards_html(result)
    rows_html = _report_rows_html(result)
    body_html = summary_html + rows_html if rows_html else summary_html + _plain_report_html(result.text or "")
    return f"""
        <html>
        <head>
        <style>
        ...
        .metric-value {{
            color: #0f172a;
            font-size: {max(font_size_pt + 3, 11)}pt;
            font-weight: 700;
            margin-top: 1pt;
        }}
        .section-title {{
            ...
            color: #0f172a;
            font-size: {max(font_size_pt + 1, 10)}pt;
            font-weight: 700;
            margin: 10pt 0 0 0;
            padding: 4pt 6pt;
        }}
...
def report_print_document(
    result: ReportResult,
    printer: QPrinter | None = None,
    *,
    font_size_pt: int = REPORT_PRINT_FONT_SIZE,
) -> QTextDocument:
    font_size_pt = max(7, min(int(font_size_pt), 18))
    doc = QTextDocument()
    if printer is not None:
        doc.documentLayout().setPaintDevice(printer)
    doc.setDefaultFont(QFont("Arial", font_size_pt))
    doc.setHtml(_report_print_html(result, font_size_pt))
    return doc


def _printer_points(printer: QPrinter, points: float) -> float:
    return points * max(printer.resolution(), 72) / 72.0
...
def _draw_report_page_chrome(
    ...
    *,
    ...
    title_font = QFont("Arial", max(font_size_pt + 5, 14))
    title_font.setBold(True)
    meta_font = QFont("Arial", max(font_size_pt - 1, 8))
    footer_font = QFont("Arial", max(font_size_pt - 1, 8))
    generated = result.generated_at.strftime(PY_DATE_DISPLAY_FORMAT + " %I:%M %p")
    company = _report_text_meta(result.text or "", "Company")
    period = _report_text_meta(result.text or "", "Period")

    header_top = page_rect.top()
    title_rect = QRectF(page_rect.left(), header_top, page_rect.width() * 0.58, header_height * 0.55)
    meta_rect = QRectF(page_rect.left() + page_rect.width() * 0.58, header_top, page_rect.width() * 0.42, header_height * 0.55)
    sub_rect = QRectF(page_rect.left(), header_top + header_height * 0.5, page_rect.width(), header_height * 0.45)

    painter.setPen(QColor("#0f172a"))
    painter.setFont(title_font)
    painter.drawText(title_rect, Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, result.title or "CRM Report")
    painter.setFont(meta_font)
    painter.setPen(QColor("#475569"))
    painter.drawText(meta_rect, Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter, f"Generated: {generated}")
    details = " | ".join(part for part in (company, f"Period: {period}" if period else "") if part)
    if details:
        painter.drawText(sub_rect, Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, details)

    line_y = page_rect.top() + header_height
    painter.setPen(QColor("#cbd5e1"))
    painter.drawLine(int(page_rect.left()), int(line_y), int(page_rect.left() + page_rect.width()), int(line_y))

    footer_top = page_rect.top() + page_rect.height() - footer_height
    painter.drawLine(int(page_rect.left()), int(footer_top), int(page_rect.left() + page_rect.width()), int(footer_top))
    painter.setFont(footer_font)
    painter.setPen(QColor("#475569"))
    footer_rect = QRectF(page_rect.left(), footer_top, page_rect.width(), footer_height)
    painter.drawText(footer_rect, Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, company or "Real Estate CRM")
    painter.drawText(
        footer_rect,
...
def print_report_document(
    ...
    *,
    ...
    if not page_rect.isValid():
        report_print_document(result, printer, font_size_pt=font_size_pt).print_(printer)
        return

    header_height = _printer_points(printer, 42)
    header_gap = _printer_points(printer, 8)
    footer_height = _printer_points(printer, 24)
    content_top = page_rect.top() + header_height + header_gap
    content_height = page_rect.height() - header_height - header_gap - footer_height
    if content_height <= _printer_points(printer, 80):
        report_print_document(result, printer, font_size_pt=font_size_pt).print_(printer)
        return

    doc = report_print_document(result, printer, font_size_pt=font_size_pt)
    doc.setPageSize(QSizeF(page_rect.width(), content_height))
    document_height = max(doc.documentLayout().documentSize().height(), content_height)
    total_pages = max(1, doc.pageCount(), math.ceil(document_height / content_height))
    ...
    try:
        for page_index in range(total_pages):
            ...
            painter.save()
            painter.setClipRect(QRectF(page_rect.left(), content_top, page_rect.width(), content_height))
            painter.translate(page_rect.left(), content_top - page_index * content_height)
            context = QAbstractTextDocumentLayout.PaintContext()
            context.clip = QRectF(0, page_index * content_height, page_rect.width(), content_height)
...
class ReportPrintPreviewDialog(QDialog):
    ...
    def __init__(self, result: ReportResult, parent: QWidget | None = None):
        ...
        self.setWindowTitle(f"Print Preview - {result.title}")
        ...
        for label, page_size in self.PAGE_SIZES:
            self.paper_combo.addItem(label, page_size)
        ...
        self.preview.paintRequested.connect(self.render_preview)
        ...
        for widget in (self.paper_combo, self.orientation_combo, self.font_spin, self.margin_spin):
    ...
    def selected_page_size(self) -> QPageSize.PageSizeId:
        return self.paper_combo.currentData() or QPageSize.PageSizeId.A4
    ...
    def apply_print_settings(self) -> None:
    ...
    def update_preview(self, *_args: Any) -> None:
        self.apply_print_settings()
        self.preview.updatePreview()

    def render_preview(self, printer: QPrinter) -> None:
        self.apply_print_settings()
        print_report_document(
            self.result,
            printer,
            font_size_pt=self.font_spin.value(),
        )

    def print_report(self) -> None:
        self.apply_print_settings()
        dialog = QPrintDialog(self.printer, self)
        dialog.setWindowTitle(f"Print {self.result.title}")
...
def print_report_result(result: ReportResult, parent: QWidget) -> None:
    ReportPrintPreviewDialog(result, parent).exec()
...
class ReportsModule(QWidget):
    def __init__(self, main: "ModernCRMWindow"):
        ...
        self.preview.setFont(report_preview_font())
        layout.addWidget(self.preview, 1)
    ...
    def print_report(self) -> None:
        if not self.last_report:
            ...
            return
        print_report_result(self.last_report, self)
...
class AIInsightsModule(QWidget):
    def __init__(self, main: "ModernCRMWindow"):
        ...
        self.preview.setFont(report_preview_font())
        layout.addWidget(self.preview, 1)
        self.preview.setPlainText("Click Refresh AI to generate insights.")
...
class ReportPreviewDialog(QDialog):
    ...
    def print_report(self) -> None:
        print_report_result(self.result, self)
    ...
    def voucher_html(self, rows: list[dict]) -> str:
        ...
        body_parts = [
            ...
            * { box-sizing: border-box; }
            body { font-family: Arial, sans-serif; color: #111827; margin: 0; font-size: 10.5pt; }
            ...
            .brand p { margin: 4px 0 0 0; color: #475569; font-size: 10pt; }
            .stamp { display: table-cell; text-align: right; vertical-align: top; font-size: 10pt; color: #475569; }
            ...
            .summary td { border: 1px solid #cbd5e1; padding: 6px 8px; font-size: 10pt; overflow-wrap: anywhere; word-break: break-word; }
            ...
            .fields th { background: #eaf2ff; color: #0f172a; border: 1px solid #bfdbfe; padding: 6px; text-align: left; font-size: 10pt; }
            .fields td { border: 1px solid #dbe3ef; padding: 6px; vertical-align: top; font-size: 9.8pt; overflow-wrap: anywhere; word-break: break-word; }
            .field-name { width: 18%; font-weight: bold; background: #f8fafc; color: #334155; }
            .footer { margin-top: 10px; font-size: 9pt; color: #64748b; text-align: center; }
            """,
            "</style></head><body>",
        ]
...
class ModernCRMWindow(QMainWindow):
    external_record_saved = Signal(str, int)
    ...
    def __init__(
        ...
        self._data_signature: tuple[tuple[str, int, int], ...] = ()
        self._external_refresh_timer: QTimer | None = None
        self._backup_timer: QTimer | None = None
        self._refresh_in_progress = False
        self._refresh_queue: list[QWidget] = []
        self._refresh_errors: list[str] = []
        self.external_record_saved.connect(self._handle_external_record_saved)
        self.local_ip = self.get_local_ip()
        self.local_api_key = os.getenv("CRM_DESKTOP_API_KEY") or secrets.token_urlsafe(24)
        ...
        self._data_signature = self.database_signature()
        self._external_refresh_timer = QTimer(self)
        self._external_refresh_timer.setInterval(5000)
        self._external_refresh_timer.timeout.connect(self.check_external_updates)
        self._external_refresh_timer.start()
        self.run_daily_backup_if_due()
        self._backup_timer = QTimer(self)
        self._backup_timer.setInterval(60 * 60 * 1000)
        self._backup_timer.timeout.connect(self.run_daily_backup_if_due)
        ...
        self.specs = {
            "rent_req": TableSpec(
                ...
                [
                    ...
                    ColumnSpec("size", "Size", width=110),
    ...
    def _build_ui(self) -> None:
        ...
        browser_label = QLabel(f"Browser: {self.browser_service_url}")
        browser_label.setObjectName("SidebarSubtle")
        browser_label.setWordWrap(True)
        self.sidebar_server_url_label = browser_label
        footer_layout.addWidget(browser_label)
        api_label = QLabel(f"Desktop API: {self.local_service_url}")
        api_label.setObjectName("SidebarSubtle")
        api_label.setWordWrap(True)
        self.sidebar_desktop_api_label = api_label
        ...
        self.refresh_button.clicked.connect(self.refresh_all_pages)
    ...
    def update_status_bar(self, message: str | None = None) -> None:
        ...
        self.status_api_label.setText(f"Web/API: {self.local_ip}:{LAN_WEB_PORT}/{LOCAL_SERVICE_PORT}")
    ...
    def start_local_service(self) -> None:
        ...
        class CRMApiHandler(BaseHTTPRequestHandler):
            ...
            def _allowed_origin(self) -> str:
                ...
                allowed = {
                    app.browser_service_url,
                    f"http://{app.local_ip}",
                    f"http://{app.local_ip}:80",
                    f"http://{app.local_ip}:{LAN_WEB_PORT}",
                    f"http://127.0.0.1:{LAN_WEB_PORT}",
                    f"http://localhost:{LAN_WEB_PORT}",
                    "http://127.0.0.1",
                    "http://localhost",
                }
                ...
                try:
                    ...
                    allowed_ports = {None, 80, LAN_WEB_PORT, LOCAL_SERVICE_PORT}
                    if parsed.scheme in {"http", "https"} and (ip.is_private or ip.is_loopback) and port in allowed_ports:
                        return origin
                except Exception:
                    pass
                return app.browser_service_url
            ...
            def _requires_auth(self, path: str) -> bool:
                return path not in ("/", "/index", "/health", "/healthz")

            def _check_auth(self, path: str) -> bool:
                if not self._requires_auth(path):
                    return True
                supplied = self._auth_header_value()
                if supplied and secrets.compare_digest(supplied, app.local_api_key):
                    return True
                self._send({"ok": False, "error": "desktop API key required"}, 401)
                return False
            ...
            def do_GET(self) -> None:
                ...
                if path in ("/", "/index"):
                    self._send({
                        ...
                        "version": "qt-1.0",
                        "message": "Qt CRM API is running. Data routes require X-API-Key.",
            ...
            def _write_record(self, method: str) -> None:
                ...
                try:
                    if method == "POST":
                        ...
                        app.external_record_saved.emit(table, int(new_id or 0))
                        ...
                        app.external_record_saved.emit(table, int(row_id or 0))
    ...
    def _dashboard_page(self) -> QWidget:
        ...
        bottom_layout.addWidget(self.pipeline_text, 1)
        
        self.figure = Figure(figsize=(5, 3), dpi=100)
        self.canvas = FigureCanvasQTAgg(self.figure)
        bottom_layout.addWidget(self.canvas, 2)
    ...
    def sync_property_from_availability(self, table: str, row: dict, status: str) -> int | None:
        ...
        owner_name = str(row.get("owner_name") or "").strip()
        owner_contact = str(row.get("contact") or row.get("contact_phone") or "").strip()
        area = str(row.get("size") or "").strip()
    ...
    def refresh_all_pages(self) -> None:
        if self._refresh_in_progress:
            self.update_status_bar("Refresh is already running")
            return
        self._refresh_in_progress = True
        self._refresh_errors = []
        self._refresh_queue = [widget for widget in self.pages.values() if hasattr(widget, "refresh")]
        if hasattr(self, "refresh_button"):
            self.refresh_button.setEnabled(False)
            self.refresh_button.setText("Refreshing...")
        self.update_status_bar("Refreshing CRM data...")
        try:
            if "dashboard" in self.pages:
                self.refresh_dashboard()
            self.intelligence_service = IntelligenceService(DB_PATH, currency_symbol=self.currency_symbol, company_name=self.company_name)
        except Exception as exc:
            self._refresh_errors.append(f"Dashboard: {exc}")
        QTimer.singleShot(0, self._refresh_next_page)

    def _refresh_next_page(self) -> None:
        if not self._refresh_queue:
            self._finish_refresh()
            return
        widget = self._refresh_queue.pop(0)
        try:
            widget.refresh()
        except Exception as exc:
            self._refresh_errors.append(f"{widget.__class__.__name__}: {exc}")
        remaining = len(self._refresh_queue)
        self.update_status_bar(f"Refreshing CRM data... {remaining} page(s) left")
        QTimer.singleShot(0, self._refresh_next_page)
    ...
    def _finish_refresh(self) -> None:
        ...
        if self._refresh_errors:
            QMessageBox.warning(
                self,
                "Refresh Issues",
                "Some CRM pages could not refresh:\n\n" + "\n".join(self._refresh_errors[:6]),
            )
            self.update_status_bar("Refresh completed with issues")
        else:
            self.update_status_bar("CRM data refreshed")

    def database_signature(self) -> tuple[tuple[str, int, int], ...]:
        signature: list[tuple[str, int, int]] = []
        for table in REFRESH_SIGNATURE_TABLES:
            try:
                row = self.services.fetch_one(f"SELECT COUNT(*) AS count, COALESCE(MAX(id), 0) AS max_id FROM {table}")
                if row:
                    signature.append((table, int(row.get("count") or 0), int(row.get("max_id") or 0)))
            except Exception:
                continue
        for db_file in (DB_PATH, Path(str(DB_PATH) + "-wal")):
            try:
                stat = db_file.stat()
                signature.append((db_file.name, int(stat.st_size), int(stat.st_mtime_ns)))
            except OSError:
                continue
        return tuple(signature)

    def check_external_updates(self) -> None:
        signature = self.database_signature()
        if not signature or signature == self._data_signature:
            return
        self._data_signature = signature
        self.refresh_external_data("LAN/browser entries")

    def _handle_external_record_saved(self, table: str, row_id: int) -> None:
        self._data_signature = self.database_signature()
        label = table.replace("_", " ")
        if row_id:
            label += f" #{row_id}"
        self.refresh_external_data(label)

    def refresh_external_data(self, source: str) -> None:
        if self._refresh_in_progress:
            self.update_status_bar(f"External update from {source}; manual refresh still running")
            return
        errors: list[str] = []
        try:
            if "dashboard" in self.pages:
            ...
            if current_widget is not None and hasattr(current_widget, "refresh"):
                try:
                    current_widget.refresh()
                except Exception as exc:
                    errors.append(f"{current_widget.__class__.__name__}: {exc}")
            if errors:
                print("External refresh issue: " + "; ".join(errors[:4]))
                self.update_status_bar("External entries detected; refresh had issues")
            else:
                self.update_status_bar(f"Desktop refreshed from {source}")
        finally:
            self._data_signature = self.database_signature()
    ...
    def refresh_dashboard(self) -> None:
        ...
        if hasattr(self, "figure"):
            ...
            categories = ['Rent Req', 'Rent Avail', 'Sale Req', 'Sale Avail']
            counts = [
                self.count('rent_requirements'),
                self.count('rent_availability'),
                self.count('sale_requirements'),
                self.count('sale_availability')
            ]
            colors = ['#3b82f6', '#10b981', '#f59e0b', '#ef4444']
            ax.bar(categories, counts, color=colors)
            ax.set_title("Pipeline Volume")
    ...
    def run_daily_backup_if_due(self) -> None:
        ...
        if self.services.settings_get("last_auto_backup_date") == today:
            return
        backup_dir = OUTPUT_DIR / "backups"
        backup_dir.mkdir(parents=True, exist_ok=True)
        path = backup_dir / f"desktop_auto_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
        try:
            with sqlite3.connect(DB_PATH) as source, sqlite3.connect(path) as destination:
                source.backup(destination)
            self.services.settings_set("last_auto_backup_date", today)
            self.services.settings_set("last_auto_backup_path", str(path))
            self.update_status_bar("Daily database backup saved")
        except Exception as exc:
...
""",
    ...
    def restart_app(self) -> None:
        if self._external_refresh_timer:
...
def deal_insert_columns(name_key: str, property_key: str, amount_key: str) -> list[str]:
    return [
        "date", name_key, "client_status", "contact", property_key, "size", amount_key,
...
def owner_broker_availability_insert_columns(name_key: str, property_key: str, amount_key: str) -> list[str]:
    return [
        "date", name_key, "contact", property_key, "size", amount_key,
