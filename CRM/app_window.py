...
from CRM.modules import DealModule, FinancialModule, EmployeesModule, SuccessFactorsModule, WorkflowModule, ReportsModule, AIInsightsModule, UsersModule, SettingsModule, DataTablePage, PhaseOneDesk, InstallmentModule, CommissionModule
...
class ModernCRMWindow(QMainWindow):
    ...
    def _build_pages(self) -> None:
        ...
        if has_permission(self.role, "financial") or has_permission(self.role, "financial_view"):
            ...
            operation_keys.append("installments")
            operation_keys.append("commissions")
        if has_permission(self.role, "employees") or has_permission(self.role, "employees_view"):
        ...
        if "installments" in operation_keys:
            self._report_startup(87, "Loading installment tracking")
            self._add_page("installments", "Installments", InstallmentModule(self, self.services))
        if "commissions" in operation_keys:
            self._report_startup(87, "Loading commission tracking")
            self._add_page("commissions", "Commissions", CommissionModule(self, self.services))
